...
int main (int ac, char* av[]) {
  ...
  try {
    ...
    desc.add_options()
      ...
      ("b-propagator-filename", po::value< string >(&bpropfilename), "file name of the b-propagator")
      ...
      ("b-propagator-pos", po::value< int >(&bproppos)->default_value(0), "position of b-propagator binary data in lime file")
  ...
  strcpy(fn, bpropfilename.c_str());
  cout << "Reading b-propagator from file " << bpropfilename << endl;
  ...
  for(int tt = 0; tt < T; tt++) {
    ...
    ofs << tt << " " << real(inner_prod(conj(bvr), vr))/svol*2*2*kappa*kappa << endl;
  }
  ofs.close();
  ...
  double cor[T];
  cout << "Computing 3-pt function for O_munukappa" << endl;
  DmuDnu(cor, v, gv, config, T, L, 0, 0, 0);
  ostringstream oss2;
  oss2 << "DrmuDrnu.";
  prev2 = oss2.width(2);
  prev = oss2.fill('0'); 
